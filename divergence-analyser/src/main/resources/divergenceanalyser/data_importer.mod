...
set BRANCHZNULL := {(qq,m,n) in BRANCHCC_PENALIZED: 
                    (branch_R[1,qq,m,n]*substation_Vnomi[1,branch_subex[1,qq,m,n]]^2/base100MVA)^2
                   +(branch_X[1,qq,m,n]*substation_Vnomi[1,branch_subex[1,qq,m,n]]^2/base100MVA)^2 <= Znull^2}; # Without per unit
...
set BUSCC_PV_UNITS := setof {(g,n) in UNITCC_PV} n;
set BUSCC_PV_SVC := setof {(svc,n) in SVCCC_PV} n;
...
param branch_X_mod{(qq,m,n) in BRANCHCC_PENALIZED} :=
  if ((qq,m,n) in BRANCHZNULL and branch_X[1,qq,m,n] >= 0) then Znull
  else if ((qq,m,n) in BRANCHZNULL and branch_X[1,qq,m,n] < 0) then -Znull
  else if (branch_X[1,qq,m,n] == 0) then Znull
