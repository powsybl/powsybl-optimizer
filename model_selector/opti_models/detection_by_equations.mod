...
var Red_Tran_Act_Dir_dbe{(qq,m,n) in BRANCHCC} =
...
var Red_Tran_Rea_Dir_dbe{(qq,m,n) in BRANCHCC} = 
...
var Red_Tran_Act_Inv_dbe{(qq,m,n) in BRANCHCC} = 
...
var Red_Tran_Rea_Inv_dbe{(qq,m,n) in BRANCHCC} =
...
subject to ctr_balance_P_dbe{PROBLEM_DETECTION_EQUATIONS, k in BUSCC diff {null_phase_bus}}:
  # Flows
    sum{(qq,k,n) in BRANCHCC} base100MVA * V[k] * Red_Tran_Act_Dir_dbe[qq,k,n]
  + sum{(qq,m,k) in BRANCHCC} base100MVA * V[k] * Red_Tran_Act_Inv_dbe[qq,m,k]
  # Generating units
...
subject to ctr_balance_Q_dbe{PROBLEM_DETECTION_EQUATIONS, k in BUSCC_PQ}: 
  # Flows
    sum{(qq,k,n) in BRANCHCC} base100MVA * V[k] * Red_Tran_Rea_Dir_dbe[qq,k,n]
  + sum{(qq,m,k) in BRANCHCC} base100MVA * V[k] * Red_Tran_Rea_Inv_dbe[qq,m,k]
  # Senerating units
