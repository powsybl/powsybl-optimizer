...
var Act_branch_shunt_1{(qq,m,n) in BRANCH_WITH_SHUNT_1} =
...
var Rea_branch_shunt_1{(qq,m,n) in BRANCH_WITH_SHUNT_1} = 
  - (branch_Ror[qq,m,n] + sigma1[qq,m,n])**2 * V[m] *
...
var Act_branch_shunt_2{(qq,m,n) in BRANCH_WITH_SHUNT_2} =
  V[n] * 
...
var Rea_branch_shunt_2{(qq,m,n) in BRANCH_WITH_SHUNT_2} = 
  - V[n] *
...
subject to ctr_balance_P_dbp{PROBLEM_DETECTION_PARAM,k in BUSCC diff {null_phase_bus}}:
  ...
  + sum{(qq,k,n) in BRANCH_WITH_SHUNT_1} base100MVA * V[k] * Act_branch_shunt_1[qq,k,n]
  + sum{(qq,m,k) in BRANCH_WITH_SHUNT_2} base100MVA * V[k] * Act_branch_shunt_2[qq,m,k]
  # Generating units
...
subject to ctr_balance_Q_dbp{PROBLEM_DETECTION_PARAM,k in BUSCC_PQ}: 
  ...
  # Flows on branches with a shunt on earth # TODO : CHANGE THE COMMENT HERE
  + sum{(qq,k,n) in BRANCH_WITH_SHUNT_1} base100MVA * V[k] * Rea_branch_shunt_1[qq,k,n]
  + sum{(qq,m,k) in BRANCH_WITH_SHUNT_2} base100MVA * V[k] * Rea_branch_shunt_2[qq,m,k]
  # Senerating units
...
minimize problem_dbp:
  ...
  + 100 * sum{(qq,m,n) in BRANCHCC_PENALIZED} b_sigma2[qq,m,n] # We do not want to move admittance too easily.
