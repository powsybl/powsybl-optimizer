...
# Get the maximum of each parameter
...
# Get absolute mean of each parameter
...
var teta{n in BUSCC} <= 10, >= -10;
var V{n in BUSCC} >= 0.5, <= 1.5; 
...
#
# The bornes have been chosen with job knowledge. Could be nonoptimal.
#
...
#
# Active Balance for all buses except slack busls
#
...
param alpha_default := 100000;

# Use the max for the alpha
param alpha_s1 = if abs_mean_targetV != 0 then 1 / abs_mean_targetV else alpha_default; 
param alpha_sigma1 = if abs_mean_branch_Ror != 0 then 1 / abs_mean_branch_Ror else alpha_default;
param alpha_sigma2 = if abs_mean_branch_admi != 0 then 1 / abs_mean_branch_admi else alpha_default;
param alpha_sigma3 = if abs_mean_branch_dephor != 0 then 1 / abs_mean_branch_dephor else alpha_default;
param alpha_sigma4 = if abs_mean_branch_angper != 0 then 1 / abs_mean_branch_angper else alpha_default;
param alpha_sigma5 = if abs_mean_branch_Gor != 0 then 1 / abs_mean_branch_Gor else alpha_default;
param alpha_sigma6 = if abs_mean_branch_Bor != 0 then 1 / abs_mean_branch_Bor else alpha_default;
param alpha_sigma7 = if abs_mean_branch_Gex != 0 then 1 / abs_mean_branch_Gex else alpha_default;
param alpha_sigma8 = if abs_mean_branch_Bex != 0 then 1 / abs_mean_branch_Bex else alpha_default;

# Use the mean for the alpha
# param alpha_sigma1 = if abs_mean_branch_Ror != 0 then 1 / abs_mean_branch_Ror else alpha_default;
# param alpha_sigma2 = if abs_mean_branch_admi != 0 then 1 / abs_mean_branch_admi else alpha_default;
# param alpha_sigma3 = if abs_mean_branch_dephor != 0 then 1 / abs_mean_branch_dephor else alpha_default;
# param alpha_sigma4 = if abs_mean_branch_angper != 0 then 1 / abs_mean_branch_angper else alpha_default;
...
minimize problem_divergence_analysis:
  0
  + sum{n in BUSCC_PV} (log(1 + alpha_s1 * abs(s1[n])))**2

  # Sum log of y with penal
  #+ sum{(qq,m,n) in BRANCHCC} (alpha_sigma1 * abs(sigma1[qq,m,n]))**2
  #+ sum{(qq,m,n) in BRANCHCC} (alpha_sigma2 * abs(sigma2[qq,m,n]))**2
  #+ sum{(qq,m,n) in BRANCHCC} (alpha_sigma3 * abs(sigma3[qq,m,n]))**2
  #+ sum{(qq,m,n) in BRANCHCC} (alpha_sigma4 * abs(sigma4[qq,m,n]))**2
  #+ sum{(qq,m,n) in BRANCHCC} (alpha_sigma5 * abs(sigma5[qq,m,n]))**2
  #+ sum{(qq,m,n) in BRANCHCC} (alpha_sigma6 * abs(sigma6[qq,m,n]))**2
  #+ sum{(qq,m,n) in BRANCHCC} (alpha_sigma7 * abs(sigma7[qq,m,n]))**2
  #+ sum{(qq,m,n) in BRANCHCC} (alpha_sigma8 * abs(sigma8[qq,m,n]))**2

  # Sum log of y with penal
  + sum{(qq,m,n) in BRANCHCC} (log(1 + alpha_sigma1 * abs(sigma1[qq,m,n])))**2
  + 100000 * sum{(qq,m,n) in BRANCHCC} (log(1 + alpha_sigma2 * abs(sigma2[qq,m,n])))**2
  + sum{(qq,m,n) in BRANCHCC} (log(1 + alpha_sigma3 * abs(sigma3[qq,m,n])))**2
  + sum{(qq,m,n) in BRANCHCC} (log(1 + alpha_sigma4 * abs(sigma4[qq,m,n])))**2
  + sum{(qq,m,n) in BRANCHCC} (log(1 + alpha_sigma5 * abs(sigma5[qq,m,n])))**2
  + sum{(qq,m,n) in BRANCHCC} (log(1 + alpha_sigma6 * abs(sigma6[qq,m,n])))**2
  + sum{(qq,m,n) in BRANCHCC} (log(1 + alpha_sigma7 * abs(sigma7[qq,m,n])))**2
  + sum{(qq,m,n) in BRANCHCC} (log(1 + alpha_sigma8 * abs(sigma8[qq,m,n])))**2

  # Sum log of y without penal
  #+ sum{(qq,m,n) in BRANCHCC} (log(1 + abs(sigma1[qq,m,n])))**2
  #+ sum{(qq,m,n) in BRANCHCC} (log(1 + abs(sigma2[qq,m,n])))**2
  #+ sum{(qq,m,n) in BRANCHCC} (log(1 + abs(sigma3[qq,m,n])))**2
  #+ sum{(qq,m,n) in BRANCHCC} (log(1 + abs(sigma4[qq,m,n])))**2
  #+ sum{(qq,m,n) in BRANCHCC} (log(1 + abs(sigma5[qq,m,n])))**2
  #+ sum{(qq,m,n) in BRANCHCC} (log(1 + abs(sigma6[qq,m,n])))**2
  #+ sum{(qq,m,n) in BRANCHCC} (log(1 + abs(sigma7[qq,m,n])))**2
  #+ sum{(qq,m,n) in BRANCHCC} (log(1 + abs(sigma8[qq,m,n])))**2
