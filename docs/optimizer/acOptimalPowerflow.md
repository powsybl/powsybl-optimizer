...
$$\sum\limits_{j\in v(i)} \boldsymbol{q_{ij}} = Q_i^{in} - \boldsymbol{\sigma_{i}^{Q,+}} + \boldsymbol{\sigma_{Q_i}^{-}} - \sum\limits_{g}\boldsymbol{Q_{i,g}} - \sum\limits_{b}\boldsymbol{Q_{i,b}} - \sum\limits_{vsc}\boldsymbol{Q_{i,vsc}} - \sum\limits_{s}\boldsymbol{b_{i,s}}{V_i}^2 - \sum\limits_{svc}\boldsymbol{b_{i,svc}}{V_i}^2, \quad i\in\text{BUSCC} \quad (6)$$
...
- $Q_i^{in}$ is the constant reactive power injected or consumed in bus $i$, by fixed generators and fixed shunts (see [Configuration of the run](inputs.md#configuration-of-the-run)), batteries that do not regulate voltage, loads and LCC stations.
...
- The reactive power $\boldsymbol{Q_{i,b}}$ produced by a voltage-regulating battery $b$ lies between corrected reactive limits derived from its reactive capability diagram at its (fixed) active power $P_b^0$, computed as for units (see [P/Q unit domain](preprocessing.md#pq-unit-domain)).
