...
| `penalty_voltage_target_ratio`              | Weight of the voltage target ratio term (Vmin/Vmax normalized target) in the ACOPF objective. If left unset, resolved at export to $1$ when `objective_choice` $=1$, and $0.01$ otherwise (see [AC optimal powerflow](acOptimalPowerflow.md)) | objective-dependent ($1$ or $0.01$) | $\mathbb{R}^{+}$                              |
| `penalty_voltage_target_data`               | Weight of the voltage target data term (V0 input value targeting) in the ACOPF objective. If left unset, resolved at export to $1$ when `objective_choice` $=2$, and $0.01$ otherwise (see [AC optimal powerflow](acOptimalPowerflow.md)) | objective-dependent ($1$ or $0.01$) | $\mathbb{R}^{+}$                              |
