...
# "Pf_branch","Pf_firstbus","Pf_secondbus" are not directly loaded from data.
...
# "Qf_branch","Qf_firstbus","Qf_secondbus" are not directly loaded from data.
# They are assigned the bus/branch number (see ampl_network_buses.txt, ampl_network_branches.txt) 
...
# "P_bus" is not directly loaded from data, but is assigned the bus number (see ampl_network_buses.txt)
...
# "Q_bus" is not directly loaded from data, but is assigned the bus number (see ampl_network_buses.txt)
...
# "V_bus" is not directly loaded from data, but is assigned the bus number (see ampl_network_buses.txt)
...
# [variant, substation]
# The 1st column "variant" (often denoted by letter "t") may be used to define time step, 
# in case this PowSyBl format is used for multi-timestep OPF (unused feature in the state estimator)

set SUBSTATIONS dimen 2; 
...
param branch_ptrDeph      {BRANCH} integer; # Number of the phase tap changer if any (-1 otherwise)
...
set BRANCHCC_TRANSFO_AS_LINES := {(qq,m,n) in BRANCHCC_FULL diff BRANCHCC_TRANSFORMER: 
                                branch_name[1,qq,m,n] == "isTwoWindingTransformer"};
...
param branch_Ror_SI {(qq,m,n) in BRANCHCC_FULL} =
  ...
  * ( if ((qq,m,n) in BRANCHCC_TRANSFO_AS_LINES) # Recall : BRANCHCC_TRANSFO_AS_LINES does not intersect BRANCHCC_REGL nor BRANCHCC_DEPH
