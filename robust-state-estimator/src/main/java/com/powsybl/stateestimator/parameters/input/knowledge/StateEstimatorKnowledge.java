...
public class StateEstimatorKnowledge {
    ...
    private void checksToAddBranchMeasure(Integer measurementNumber, Map<String, String> measure, Network network)
        ...
        if (!measure.containsKey("Value")) {
            throw new IllegalArgumentException("No entry corresponding to the measurement value (recall : in MW or MVar).");
        }
        if (!measure.containsKey("Variance")) {
            throw new IllegalArgumentException("No entry corresponding to the measurement variance (recall : in MW^2 or MVar^2).");
        }
        ...
        if (network.getBranchStream().noneMatch(b -> b.getId().equals(measure.get("BranchID")))) {
            throw new IllegalArgumentException("The branch ID related to the measurement does not exist in the network.");
        }
        if (network.getBusView().getBusStream().noneMatch(b -> b.getId().equals(measure.get("FirstBusID")))) {
            throw new IllegalArgumentException("The ID of the first bus related to the measurement does not exist in the network.");
        }
        if (network.getBusView().getBusStream().noneMatch(b -> b.getId().equals(measure.get("SecondBusID")))) {
    ...
    private StateEstimatorKnowledge addActivePowerFlowMeasure(Integer measurementNumber, Map<String, String> measure, Network network) {
        // Consistency checks on the measurement provided
        checksToAddBranchMeasure(measurementNumber, measure, network);
        // Check uniqueness of measurement number (for Pf measurements)
        if (activePowerFlowMeasures.containsKey(measurementNumber)) {
        ...
        if (activePowerFlowMeasures.values()
                .stream()
                .anyMatch(meas -> meas.get(1).equals(measure.get("BranchID"))
                        && meas.get(2).equals(measure.get("FirstBusID")))) {
    ...
    private StateEstimatorKnowledge addReactivePowerFlowMeasure(Integer measurementNumber, Map<String, String> measure, Network network) {
        // Consistency checks on the measurement provided
        checksToAddBranchMeasure(measurementNumber, measure, network);
        // Check uniqueness of measurement number (for Qf measurements)
        if (reactivePowerFlowMeasures.containsKey(measurementNumber)) {
        ...
        // Check that the measurement is not already contained in reactivePowerFlowMeasures (it is sufficient to check that the pair (BranchID, FirstBusID) does not already exist)
        if (reactivePowerFlowMeasures.values()
                .stream()
                .anyMatch(meas -> meas.get(1).equals(measure.get("BranchID"))
                        && meas.get(2).equals(measure.get("FirstBusID")))) {
            throw new IllegalArgumentException("A measurement already exists for the location and type of the measurement provided. It can not be added.");
    ...
    private void checksToAddBusMeasure(Integer measurementNumber, Map<String, String> measure, Network network)
            throws IllegalArgumentException {
        // Consistency checks on the measurement provided
        if (measurementNumber < 1) {
            throw new IllegalArgumentException("The measurement number must be a nonzero integer.");
        ...
        if (!measure.containsKey("Value")) {
            throw new IllegalArgumentException("No entry corresponding to the measurement value (recall : in MW, MVar or kV).");
        }
        if (!measure.containsKey("Variance")) {
            throw new IllegalArgumentException("No entry corresponding to the measurement variance (recall : in MW^2, MVar^2 or kV^2).");
        }
        ...
        if (network.getBusView().getBusStream().noneMatch(b -> b.getId().equals(measure.get("BusID")))) {
    ...
    private StateEstimatorKnowledge addActivePowerInjectedMeasure(Integer measurementNumber, Map<String, String> measure, Network network) {
        // Consistency checks on the measurement provided
        checksToAddBusMeasure(measurementNumber, measure, network);
        // Check uniqueness of measurement number (for P measurements)
        if (activePowerInjectedMeasures.containsKey(measurementNumber)) {
        ...
        if (activePowerInjectedMeasures.values()
                .stream()
                .anyMatch(meas -> meas.get(1).equals(measure.get("BusID")))) {
            throw new IllegalArgumentException("A measurement already exists for the location and type of the measurement provided. It can not be added.");
        }
        // Add the measurement to the list
        ArrayList<String> arrayMeasure = new ArrayList<>();
    ...
    private StateEstimatorKnowledge addReactivePowerInjectedMeasure(Integer measurementNumber, Map<String, String> measure, Network network) {
        // Consistency checks on the measurement provided
        checksToAddBusMeasure(measurementNumber, measure, network);
        // Check uniqueness of measurement number (for Q measurements)
        if (reactivePowerInjectedMeasures.containsKey(measurementNumber)) {
        ...
        if (reactivePowerInjectedMeasures.values()
                .stream()
                .anyMatch(meas -> meas.get(1).equals(measure.get("BusID")))) {
            throw new IllegalArgumentException("A measurement already exists for the location and type of the measurement provided. It can not be added.");
        }
    ...
    private StateEstimatorKnowledge addVoltageMagnitudeMeasure(Integer measurementNumber, Map<String, String> measure, Network network) {
        // Consistency checks on the measurement provided
        checksToAddBusMeasure(measurementNumber, measure, network);
        // Check uniqueness of measurement number (for V measurements)
        if (voltageMagnitudeMeasures.containsKey(measurementNumber)) {
        ...
        if (voltageMagnitudeMeasures.values()
                .stream()
                .anyMatch(meas -> meas.get(1).equals(measure.get("BusID")))) {
            throw new IllegalArgumentException("A measurement already exists for the location and type of the measurement provided. It can not be added.");
        }
    ...
    public StateEstimatorKnowledge setSlack(String slackBusId, Network network) {
        ...
        } else // If slack bus ID is directly given, check that its ID does exist in the network
            if (!network.getBusView().getBusStream().map(Identifiable::getId).toList().contains(slackBusId)) {
    ...
    public StateEstimatorKnowledge setStateVectorStartingPoint(Map<String, Pair<Double, Double>> startingPoint, Network network) {
        ...
        if (startingPoint == null) {
            ...
            for (Bus bus : network.getBusView().getBuses()) {
                ...
                } else { // Else, write a "flat start" for the bus
